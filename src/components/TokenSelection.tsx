...
const TokenSelection = () => {
  ...
  const validateToken = async () => {
    ...
    try {
      ...
      console.log('✅ Token validated:', info);
  ...
  return (
    <div style={{backgroundColor: '#1A202C'}} className="min-h-screen pt-2">
          ...
          <div className="space-y-3">
            <div className="relative">
              <input
                ...
                style={{backgroundColor: '#4A5568', borderColor: getInputBorderColor()}}
              ...
              <div style={{backgroundColor: '#4A5568'}} className="rounded-lg p-3 border border-green-500">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center">
                    {tokenInfo.logoURI && (
                      <img src={tokenInfo.logoURI} alt={tokenInfo.symbol} className="w-8 h-8 rounded-full mr-3" />
                    )}
                    <div>
                      <div className="text-white font-bold">{tokenInfo.symbol}</div>
                      <div className="text-gray-300 text-sm">{tokenInfo.name}</div>
                    </div>
                  </div>
                  <div className="flex space-x-2">
                    ...
                    <span className="bg-green-500 text-white px-2 py-1 rounded text-xs">Tradeable</span>
                  </div>
                </div>
      ...
      <BotConfiguration tokenInfo={tokenInfo} />
      <ExecutionModes tokenInfo={tokenInfo} />
    </div>
  );
};
