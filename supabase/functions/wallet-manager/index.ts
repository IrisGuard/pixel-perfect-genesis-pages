...
import { JsonRpcProvider, Wallet as EvmWallet, formatEther, parseEther, Contract, parseUnits } from "https://esm.sh/ethers@6.13.4?bundle&target=denonext";
import {
  Keypair as SolKeypair,
  Connection as SolConnection,
  VersionedTransaction as SolVersionedTx,
...
const SolTx = SolTransaction;
const SolPubKey = SolPublicKey;
const solSend = solSendAndConfirm;
const LPSOL = LAMPORTS_PER_SOL;
const SolConn = SolConnection;
const SolPK = SolPublicKey;
const SolKP = SolKeypair;
const solSendConfirm = solSendAndConfirm;
const LSOL = LAMPORTS_PER_SOL;

const SOLANA_TOKEN_PROGRAM_ID = new SolPublicKey("TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA");
const SOLANA_TOKEN_2022_PROGRAM_ID = new SolPublicKey("TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb");
const SOLANA_ASSOCIATED_TOKEN_PROGRAM_ID = new SolPublicKey("ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJA8knL");
...
async function resolveTokenProgramForMint(connection: SolConnection, mintPubkey: SolPublicKey) {
  const mintAccountInfo = await connection.getAccountInfo(mintPubkey);
  if (!mintAccountInfo) {
    throw new Error("Mint not found on-chain");
  }

  const isToken2022 = mintAccountInfo.owner.equals(SOLANA_TOKEN_2022_PROGRAM_ID);
  return {
    isToken2022,
    tokenProgramId: isToken2022 ? SOLANA_TOKEN_2022_PROGRAM_ID : SOLANA_TOKEN_PROGRAM_ID,
  };
}
  ...
  try {
    ...
    if (action === "transfer_to_master") {
      ...
      if (transfer_type === "token" && mint) {
        ...
        const { tokenProgramId, isToken2022 } = await resolveTokenProgramForMint(connection, mintPubkey);
        ...
        const rawAmount = typeof transferAmount === "number"
          ? BigInt(Math.floor(transferAmount))
          : BigInt(tokenBalance.value.amount || "0");
        if (rawAmount <= 0n) return json({ error: "Invalid token amount" }, 400);

        tx.add(createSplTransfer(sourceAta, destAta, keypair.publicKey, rawAmount, [], tokenProgramId));
        const sig = await solSendAndConfirm(connection, tx, [keypair], { commitment: "confirmed" });
        console.log(`✅ Transferred ${isToken2022 ? "Token-2022" : "SPL"} tokens to master: ${sig}`);
    ...
    if (action === "transfer_between_wallets") {
      ...
      if (transfer_type === "token" && mint) {
        ...
        tx.add(createSplTransfer(sourceAta, destAta, keypair.publicKey, rawAmount, [], tokenProgramId));
        const sig = await solSendAndConfirm(connection, tx, [keypair], { commitment: "confirmed" });
        return json({ success: true, signature: sig, amount: Number(rawAmount), token_program: isToken2022 ? "token-2022" : "spl" });
      }
    ...
    if (action === "check_balances") {
      const requestedWalletIds = Array.isArray(body.wallet_ids)
        ? body.wallet_ids.filter((value: unknown): value is string => typeof value === "string" && value.length > 0)
        : [];
      ...
      if (requestedWalletIds.length > 0) {
        const { data, error } = await supabase
          .from("admin_wallets")
          .select("id, public_key, is_master, wallet_type, wallet_index")
          .eq("network", network)
      ...
      if (wallets.length === 0) return json({ balances: [], tokenBalances: {}, tokenMeta: {} });
      ...
      if (isEvm) {
        ...
        // Check others in batches with delays
        for (let i = 0; i < otherWallets.length; i += 5) {
          const batch = otherWallets.slice(i, i + 5);
          const batchPromises = batch.map(async (w: any) => {
        ...
        // If none to check, skip token scan (still return native balances)
        if (evmWalletsToCheck.length === 0) {
          return json({ balances, tokenBalances, tokenMeta });
        }

        const rpc = rpcs[0];

        // Discover token contracts from known list + on-chain Transfer logs (last ~100k blocks)
        ...
        for (const w of evmWalletsToCheck) {
          try {
            const currentBlockRes = await fetch(rpc, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ jsonrpc: "2.0", method: "eth_blockNumber", params: [], id: 1 }),
            });
            const currentBlockData = await currentBlockRes.json();
            const currentBlock = parseInt(currentBlockData.result || "0x0", 16);
            if (currentBlock <= 0) continue;

            for (const blockRange of [100000, 50000, 20000, 10000]) {
                ...
                const logsRes = await fetch(rpc, {
                  ...
                  body: JSON.stringify({
                    ...
                    params: [{
                      ...
                      toBlock: "latest",
                      topics: [ERC20_TRANSFER_TOPIC, null, [
                        "0x000000000000000000000000" + w.public_key.slice(2).toLowerCase(),
                        null,
                      ]],
                    }],
                    id: 1,
            ...
            // Skip log discovery for this wallet if provider rejects it
          }
        }
        ...
        for (const dt of discoveredTokens) {
          if (!allTokenAddresses.has(dt)) {
            allTokenAddresses.set(dt, { symbol: "", name: "", decimals: 18 });
          }
        }
      ...
      // 2. Fetch SPL token balances for master + sub-treasury wallets, or targeted wallets after a swap
      ...
      for (let i = 0; i < walletsWithPossibleFunds.length; i += BATCH_SIZE) {
        ...
        const results = await Promise.allSettled(batch.map(async (maker) => {
          try {
            ...
            // Check if wallet has any token accounts (to decide if it becomes a "holding" wallet)
