  ...
  for (const slip of (isBuyPool ? [5000, 7500, 9500] : [3000, 5000, 7500])) {
    try {
      ...
      const swapRes = await fetch("https://lite-api.jup.ag/swap/v1/swap", {
        ...
        body: JSON.stringify({
          quoteResponse: quote, userPublicKey: params.wallet, wrapAndUnwrapSol: true,
          dynamicComputeUnitLimit: true, prioritizationFeeLamports: 10000,
        }),
      });
      if (!swapRes.ok) continue;
      const swapData = await swapRes.json();
...
function scheduleNextTrade(supabaseUrl: string, delayMs: number, sessionId?: string) {
  ...
  const selfUrl = `${supabaseUrl}/functions/v1/dex-volume-bot`;
